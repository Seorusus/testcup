...
class UpdateKernel extends DrupalKernel {
  ...
  protected function handleAccess(Request $request) {
    ...
    if (!Settings::get('update_free_access', FALSE) && !$db_update_access->access($account)->isAllowed()) {
      throw new AccessDeniedHttpException('In order to run update.php you need to either have "Administer software updates" permission or have set $settings[\'update_free_access\'] in your _settings.php.');
    }
  }
